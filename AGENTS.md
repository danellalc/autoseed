...
- The MegaMart torture model (self-reference, embedded struct, composite and shared primary keys, unique columns, soft-delete, many2many, correlated derived values, all in one seed run) must stay green. Composite unique constraints and a ternary/N-ary attributed join (3+ required references sharing a key) are modeled and tested, but in their own dedicated fixtures (`PGInvoice`/`PGStudentCourseTeacher` in gormseed, `Invoice`/`Assignment` in entseed), not folded into MegaMart itself.
