...
// LocaleCustomer exercises every field kind WithLocale("pt_BR") claims,
// in one real GORM model.
type LocaleCustomer struct {
...
// WithLocale("pt_BR"): every claimed field's real, generated-and-stored
// value has the shape the pt_BR rules promise, and EmailRule's existing
// sibling correlation (unmodified by locale support) still agrees with
// whatever Brazilian FirstName/LastName landed on the same row.
func TestSeed_Postgres_LocalePtBR(t *testing.T) {
	db := postgresDB(t)
	models := []any{&LocaleCustomer{}}
	if err := db.AutoMigrate(models...); err != nil {
		t.Fatalf("AutoMigrate: %v", err)
	}

	if err := gormseed.Seed(context.Background(), db, models, autoseed.WithSeed(5), autoseed.WithScale(30), autoseed.WithLocale("pt_BR")); err != nil {
		t.Fatalf("Seed: %v", err)
	}

	var customers []LocaleCustomer
	db.Find(&customers)
	if len(customers) != 30 {
		t.Fatalf("count = %d, want 30", len(customers))
	}
