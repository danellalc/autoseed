...
// field's ent name -- Number declares an explicit StorageKey that
// differs from "number", so this also guards that resolution goes
// through StorageKey rather than assuming it equals the field's Name.
// Neither field alone is marked Field.Unique -- Invoice.Series and
// Invoice.Number are only unique together.
