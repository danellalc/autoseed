...
func (_q *EmployeeQuery) QueryReports() *EmployeeQuery {
	query := (&EmployeeClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *EmployeeQuery) WithReports(opts ...func(*EmployeeQuery)) *EmployeeQuery {
	query := (&EmployeeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EmployeeQuery) loadManager(ctx context.Context, query *EmployeeQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *Employee)) error {
	...
	for i := range nodes {
		if nodes[i].employee_reports == nil {
			continue
		}
		fk := *nodes[i].employee_reports
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "employee_reports" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *EmployeeQuery) loadReports(ctx context.Context, query *EmployeeQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *Employee)) error {
	...
	for _, n := range neighbors {
		fk := n.employee_reports
		if fk == nil {
			return fmt.Errorf(`foreign-key "employee_reports" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "employee_reports" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
