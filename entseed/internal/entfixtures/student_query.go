...
func (_q *StudentQuery) QueryAssignments() *AssignmentQuery {
...
func (_q *StudentQuery) WithAssignments(opts ...func(*AssignmentQuery)) *StudentQuery {
	query := (&AssignmentClient{config: _q.config}).Query()
...
func (_q *StudentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Student, error) {
	...
	if query := _q.withAssignments; query != nil {
		if err := _q.loadAssignments(ctx, query, nodes,
			func(n *Student) { n.Edges.Assignments = []*Assignment{} },
			func(n *Student, e *Assignment) { n.Edges.Assignments = append(n.Edges.Assignments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *StudentQuery) loadAssignments(ctx context.Context, query *AssignmentQuery, nodes []*Student, init func(*Student), assign func(*Student, *Assignment)) error {
	...
	query.Where(predicate.Assignment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(student.AssignmentsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.student_assignments
		if fk == nil {
			return fmt.Errorf(`foreign-key "student_assignments" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "student_assignments" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
